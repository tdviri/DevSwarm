...
export default function UserAnsweredQuestions(props) {
  ...
  if (props.userAnsweredQuestions && props.userAnsweredQuestions.length > 0) {
    questionData = props.userAnsweredQuestions
  ...
  const handleNext = () => {
    if (startIndex + 5 < props.userAnsweredQuestions.length) {
        ...
        <div className="posts-container">
          ...
          {props.userAnsweredQuestions.map((question, index) => (
            <div key={index} id={`post${index}`} className="post">
              ...
              <div id={`post-title${index}`} className="title-and-tags">
                <span
                  onClick={() => {
                    props.setDisplayUserAnswers(true);
                    props.handleAnswerPage(
                      index,
                      props.userAnsweredQuestions,
                      true,
                    );
                  }}
          ...
          {props.userAnsweredQuestions.length > 5 && (
            <div className="pagination">
              ...
              <button
                onClick={handleNext}
                disabled={startIndex + 5 >= props.userAnsweredQuestions.length}
              >
                Next
              </button>
            </div>
