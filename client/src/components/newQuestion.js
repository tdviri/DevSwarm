...
export default function NewQuestion(props) {
  ...
  async function handleEditQuestion(formData) {
    ...
    const loggedInUser = (
      await axios.get("http://localhost:8000/api/getLoggedInUser", {
        withCredentials: true,
        headers: {
          "Content-Type": "application/json",
        },
    ...
    await axios.put(
      "http://localhost:8000/api/replacequestion",
      { origQuestion: props.userQuestion, newQuestion: newQuestion },
      { withCredentials: true },
    );
  ...
  async function handleDeleteQuestion(formData) {
    await axios.delete("http://localhost:8000/api/deletequestion", {
      withCredentials: true,
      data: props.userQuestion,
    });
    const updatedUserQuestions = props.userQuestions.filter(
      (question) => question._id !== props.userQuestion._id,
    );
    props.setUserQuestions(updatedUserQuestions);
    props.setClickedOnProfileSidebar(true);
    props.fetchData();
    ...
    <form id="ask-question-form" className="ask-question-form">
      <div className="form-question-subheader-container">
        ...
        <input
          ...
          required
          name="title"
          defaultValue={props.userQuestion.title}
        />
      </div>
      <br />
      <br />
      ...
      <div className="form-question-subheader-container">
        ...
        <input
          ...
          name="tags"
          defaultValue={
            props.userQuestion.tags && props.userQuestion.tags.length > 0
              ? props.userTags
                  .filter((tag) => {
                    return props.userQuestion.tags.includes(tag._id);
                  })
                  .map((tag) => tag.name)
                  .join(" ")
              : ""
      ...
      <div className="submit-form">
        <input
          ...
          value="Post Edited Question"
          onClick={(e) =>
            handleEditQuestion(
              new FormData(document.getElementById("ask-question-form")),
            )
          }
        />
        <input
      ...
      <div className="ask-question-form-error-message">
        {inputValidationError && (
          <div>All fields besides Tags need to be filled</div>
        )}
        {formTitleError && (
          <div className="error-message">
            Question title cannot be more than 140 characters in length
          </div>
        )}
        {tagCountError && (
          <div className="error-message">
