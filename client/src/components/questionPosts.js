...
export default function QuestionPosts(props) {
  ...
  const questionData = props.questions
    ...
    .map((question) => {
      ...
      const elapsedTime =
        currentTime.getTime() - new Date(question.ask_date_time).getTime();
  ...
  async function handleVote(upvote, question) {
    ...
    const questionIsVoted = (
      await axios.get(
        `http://localhost:8000/api/isquestionvoted/${question._id}`,
    ...
    try {
      ...
      await axios.put("http://localhost:8000/api/addvotedquestion", data, {
        withCredentials: true,
        headers: {
  ...
  function getSortedComments(questionComments) {
    ...
    return props.comments
      .filter((comment) => questionComments.includes(comment._id))
  ...
  async function postComment(question, commentText) {
    try {
      ...
      await axios.put(
        "http://localhost:8000/api/updatequestioncomments",
        { questionId: question._id, commentId: comment._id },
        {
          withCredentials: true,
        },
      );
    ...
    <div className="posts-container">
        ...
        <div key={index} id={`post${index}`} className="post">
          <div className="upvote-downvote-arrows">
            <div
              ...
              onClick={() => handleVote(true, question)}
            >
              <FaArrowUp
                className={
            ...
            <div
              ...
              onClick={() => handleVote(false, question)}
            >
              <FaArrowDown
                className={
          ...
          <div id={`post-title${index}`} className="title-and-tags">
            <span
              onClick={() => {
                props.handleAnswerPageIndex(index, questions, true);
              }}
          ...
          {question.comments.length > 0 && (
            <div className="comments-dropdown">
              ...
              {activeDropdownId === question._id &&
                getSortedComments(question.comments)
              ...
              {question.comments.length > 3 &&
                activeDropdownId === question._id && (
                  <div className="comments-pagination-buttons">
                    ...
                    <button
                      disabled={
                        commentStartIndex + 3 >= question.comments.length
                      }
                      onClick={() =>
                        setCommentStartIndex(commentStartIndex + 3)
                      }
      ...
      {questions.length > 5 && (
        <div className="pagination">
          ...
          <button
            onClick={handleNext}
            disabled={startIndex + 5 >= questions.length}
          >
            Next
          </button>
        </div>
