...
export default function UserProfile(props) {
  ...
  const [showUserAnsweredQuestionsPage, setShowUserAnsweredQuestionsPage] =
    useState(false);
  const [showNewQuestionForm, setShowNewQuestionForm] = useState(false);
  const [displayUserDeletionWarning, setDisplayUserDeletionWarning] =
    useState(false);
  ...
  async function fetchUserData() {
    try {
      ...
      resp = await axios.get(
        "http://localhost:8000/api/getuseransweredquestions",
        { withCredentials: true },
      );
  ...
  return (
    <div className="user-profile">
      ...
      {!props.clickedOnProfileSidebar &&
        !showNewQuestionForm &&
        showUserAnsweredQuestionsPage && (
          <UserAnsweredQuestions
            handleAnswerPage={props.handleAnswerPage}
            setDisplayUserAnswers={props.setDisplayUserAnswers}
            fetchUserData={fetchUserData}
            tags={props.tags}
      ...
      {props.clickedOnProfileSidebar && (
        <div className="user-profile-container">
          ...
          <h3 className="user-profile-account-details-header">
            Account Details
          </h3>
          <div className="user-profile-reputation">
            Reputation: {userReputation}
          </div>
          <div className="user-profile-account-duration">
            Account created {userAccountDuration} days ago
          </div>
          <h3 className="user-profile-questions-asked-header">
            Questions Asked
          </h3>
          {userQuestions && userQuestions.length === 0 && (
            <div className="user-profile-no-questions-asked">
              Currently no questions asked
            </div>
          )}
          <div className="user-profile-asked-questions-list">
            {userQuestions &&
              userQuestions.map((userQuestion) => {
                return (
                  <div
                    className="user-profile-asked-question"
                    onClick={() => viewNewQuestionForm(userQuestion)}
                  >
                    {userQuestion?.title}
                  </div>
                );
          ...
          <div
            className="user-profile-view-tags"
            onClick={() => {
              viewUserTagsPage();
            }}
          >
          ...
          <div
            className="user-profile-answered-questions"
            onClick={() => viewUserAnsweredQuestionsPage()}
          >
            View Your Answered Questions
          </div>
          {props.currentLoggedInUser && props.isAdmin && (
            <h3 className="admin-profile-all-users-header">All Users</h3>
          )}
          {props.currentLoggedInUser &&
            props.isAdmin &&
            allUsers?.length > 1 && (
              <div className="admin-profile-list-of-users">
                {allUsers?.map((user, index) => (
                  <div className="admin-profile-user-container">
                    <div
                      className="admin-profile-user-info"
                      onClick={() => {
                        props.switchUser(user);
                        fetchUserData();
                        props.setCurrentLoggedInUser(user);
                        props.setShowUserProfile(false);
                      }}
                    >
                      Username: {user.username}, Email: {user.email}
                    </div>
                    <span className="admin-profile-user-info-seperator">
                      .......
                    </span>
                    {user.email !== "admin@gmail.com" &&
                      user._id !== props.currentLoggedInUser._id && (
                        <button
                          className="admin-profile-delete-user-btn"
                          onClick={() => {
                            setDisplayUserDeletionWarning(true);
                            setUserToWarn(user);
                          }}
                        >
                          Delete User
                    ...
                    {userToWarn === user && displayUserDeletionWarning && (
                      <div className="user-deletion-confirmation-container">
                        <span>Are you sure you want to remove the user?</span>
                        <button
                          className="delete-user-yes-option-btn"
                          onClick={() => {
                            deleteUser(user, index);
                            setDisplayUserDeletionWarning(false);
                            setUserToWarn(null);
                          }}
                        >
                          Yes
                        ...
                        <button
                          className="delete-user-no-option-btn"
                          onClick={() => {
                            setDisplayUserDeletionWarning(false);
                            setUserToWarn(null);
                          }}
          ...
          {props.currentLoggedInUser &&
            props.isAdmin &&
            allUsers?.length === 1 && (
              <div className="admin-profile-no-users-message">
                No other user accounts created
              </div>
            )}
        </div>
      )}
      {!props.clickedOnProfileSidebar && showNewQuestionForm && (
        <NewQuestion
          fetchUserData={fetchUserData}
          setUserQuestions={setUserQuestions}
          userQuestions={userQuestions}
          setClickedOnProfileSidebar={props.setClickedOnProfileSidebar}
          tags={props.tags}
          fetchData={props.fetchData}
          userQuestion={questionToModify}
          userTags={userTags}
        />
      )}
    </div>
