...
export default function Answers(props) {
  ...
  const [numOfAnswers, setNumOfAnswers] = useState(
    props.questions[props.answerPageIndex].answers.length,
  );
  ...
  useEffect(() => {
    ...
    let elapsedTime =
      currTime.getTime() -
      new Date(props.questions[props.answerPageIndex].ask_date_time).getTime();
    let answerPageTimeMessage;
    let startYear = new Date(
      props.questions[props.answerPageIndex].ask_date_time,
    ).getFullYear();
    ...
    const updatedAnswerPage = (
      <div>
        ...
        <div className="answers-header">
          <span className="answer-count-answers-page">
            {" "}
            {numOfAnswers} answers{" "}
          </span>
          <div className="question-title-answers-page">
            {" "}
            {props.questions[props.answerPageIndex].title}{" "}
          </div>
          {props.isLoggedIn && (
            <div>
              <button
        ...
        <div className="question-details-answers-page">
          <span className="views-count-answers-page">
            {" "}
            {props.questions[props.answerPageIndex].views} views
          </span>
          <span className="question-text-answer-page">
            {" "}
            {props.questions[props.answerPageIndex].text}{" "}
          </span>
          <div className="answer-info">
            <span className="post-username-question-answers-page">
              {" "}
              {props.questions[props.answerPageIndex].asked_by}{" "}
            </span>
            <span className="post-time-answers-page">
              {answerPageTimeMessage}
            </span>
          </div>
        </div>
      </div>
    ...
    let newAnsArray = [];
    props.questions[props.answerPageIndex].answers.forEach(
      (ansIdForQuestion) => {
    ...
    if (props.viewingUserAnswers) {
      const userAnswers = sortedAnsArray.filter((ans) =>
        loggedInUser.answersAdded.includes(ans._id),
      );
      const otherAnswers = sortedAnsArray.filter(
        (ans) => !loggedInUser.answersAdded.includes(ans._id),
      );
    ...
    sortedAnsArray.forEach((ans) => {
      if (props.questions[props.answerPageIndex].answers.includes(ans._id)) {
        const comment = props.comments.find(
          (comment) =>
            props.questions[props.answerPageIndex].answers.comments &&
            props.questions[props.answerPageIndex].answers.comments.includes(
              comment._id,
            ),
        );
        let endYear = currTime.getFullYear();
        ...
        currTime = new Date();
        let elapsedTime =
          currTime.getTime() - new Date(ans.ans_date_time).getTime();
        let startYear = new Date(ans.ans_date_time).getFullYear();

        let years = Math.abs(Math.floor(endYear - startYear));
        if (
          currTime.getMonth() < new Date(ans.ans_date_time).getMonth() ||
          (currTime.getMonth() === new Date(ans.ans_date_time).getMonth() &&
            currTime.getDate() < new Date(ans.ans_date_time).getDate())
        ) {
          years--;
        }
        ...
        const answerPost = (
          <div className="answer-post">
            <div className="answer-upvote-downvote-arrows">
              <div
                className="answer-upvote-arrow"
                onClick={() => handleAnswerVote(true, ans)}
              >
                <FaArrowUp
                  className={
                    props.isGuest ? "guest-upvote" : "authenticated-upvote"
                  }
                />
              </div>
              <div className="answer-vote-count">{ans.votes}</div>
              <div
                className="answer-downvote-arrow"
                onClick={() => handleAnswerVote(false, ans)}
              >
                <FaArrowDown
                  className={
                    props.isGuest ? "guest-downvote" : "authenticated-downvote"
                  }
                />
              </div>
            </div>
            ...
            <div className="answer-info2">
              ...
              <span className="post-time-answers-page">
                {answerPostTimeMessage}
              </span>
            </div>
            {props.isLoggedIn && (
              <div className="comment-form">
                <CommentForm ans={ans} handleFormSubmit={handleFormSubmit} />
              </div>
            )}
            {props.isLoggedIn &&
              props.viewingUserAnswers &&
              loggedInUser.answersAdded.includes(ans._id) && (
                <div className="answer-modification-buttons">
                  <button
                    className="edit-answer-btn"
                    onClick={() => editAnswer(ans)}
                  >
                    Edit
                  </button>
                  <button
            ...
            {ans.comments.length > 0 && (
              <div className="comments-dropdown">
                <div
                  className="comments-dropdown-header"
                  onClick={() => toggleDropdown(ans._id)}
                >
                  Show Comments
                </div>
                {activeDropdownId === ans._id &&
                  getSortedComments(ans.comments)
                    .slice(commentStartIndex, commentStartIndex + 3)
                        ...
                        return (
                          <div className="comment-post" key={commentIndex}>
                            <div className="comment-upvote">
                              <div
                                className="comment-upvote-arrow"
                                onClick={() => handleCommentVote(comment)}
                              >
                                <FaArrowUp
                                  className={
                                    props.isGuest
                ...
                {ans.comments.length > 3 && activeDropdownId === ans._id && (
                  <div className="comments-pagination-buttons">
                    <button
                      disabled={commentStartIndex === 0}
                      onClick={() =>
                        setCommentStartIndex(commentStartIndex - 3)
                      }
                    >
                      Prev
                    </button>
                    <button
                      disabled={commentStartIndex + 3 >= ans.comments.length}
                      onClick={() =>
                        setCommentStartIndex(commentStartIndex + 3)
                      }
                    >
  ...
  function getSortedComments(answerComments) {
    ...
    return props.comments
      .filter((comment) => answerComments.includes(comment._id))
  ...
  async function handleAnswerVote(upvote, answer) {
    ...
    const answerIsVoted = (
      await axios.get(`http://localhost:8000/api/isanswervoted/${answer._id}`, {
        withCredentials: true,
      })
    ).data;
    if (answerIsVoted) {
    ...
    try {
      ...
      await axios.put("http://localhost:8000/api/addvotedanswer", data, {
        withCredentials: true,
        headers: {
  ...
  async function handleCommentVote(comment) {
    ...
    const commentIsVoted = (
      await axios.get(
        `http://localhost:8000/api/iscommentvoted/${comment._id}`,
        { withCredentials: true },
      )
    ).data;
    if (commentIsVoted) {
    ...
    try {
      ...
      await axios.put("http://localhost:8000/api/addvotedcomment", data, {
        withCredentials: true,
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",
        },
  ...
  async function postComment(ans, commentText) {
    try {
      const resp = await axios.post(
        "http://localhost:8000/api/addcomment",
      ...
      await axios.put(
        "http://localhost:8000/api/updateanswercomments",
        { ansId: ans._id, commentId: comment._id },
        { withCredentials: true },
      );
  ...
  async function deleteAnswer(answer) {
    try {
      ...
      setAnswers((prevAnswers) =>
        prevAnswers.filter((ans) => ans._id !== answer._id),
      );
  ...
  return (
    <div>
      {displayEditAnswerForm && (
        <EditAnswerForm
          replaceWithEditedAnswer={replaceWithEditedAnswer}
          fetchData={props.fetchData}
          answerToEdit={answerToEdit}
          setDisplayEditAnswerForm={setDisplayEditAnswerForm}
        />
      )}
      {!displayEditAnswerForm && (
        <div>
          ...
          {answerPosts.length > 5 && (
            <div className="pagination-buttons">
              ...
              <button
                disabled={startIndex + 5 >= answerPosts.length}
                onClick={() => setStartIndex(startIndex + 5)}
              >
                Next
              </button>
          ...
          {props.isLoggedIn && (
            <button
              onClick={() => props.showAnswerForm(true)}
